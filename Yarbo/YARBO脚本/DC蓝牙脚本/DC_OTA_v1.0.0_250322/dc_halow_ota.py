...
def get_base_name():
    ...
    try:
        # 使用固定的绝对路径
        base_json_path = "/home/YARBO/.ros/base.json"
        ...
        if os.path.exists(base_json_path):
            ...
            logger.info(f"base.json文件不存在: {base_json_path}, 将通过扫描选择设备")
            return None
    except json.JSONDecodeError:
        logger.warning(f"base.json格式错误，无法解析")
        return None
    except Exception as e:
        logger.warning(f"读取BaseName出错: {str(e)}")
        return None
...
async def run():
    ...
    if not ble_name:
        logger.info("无法从base.json获取设备名称，将扫描所有可用设备")
        print("注意: 未能从base.json获取设备名称")
        print("将扫描所有可用蓝牙设备，您需要手动选择设备")
        
        # 扫描设备但不指定名称过滤
        scan_timeout = 10.0
        print(f"正在扫描蓝牙设备，请等待 {scan_timeout} 秒...")
        logger.info(f"扫描时间设置为: {scan_timeout}秒")
        ...
        try:
            ...
            if not devices:
                logger.error("未找到任何蓝牙设备")
                print("错误: 未找到任何蓝牙设备，请确保蓝牙已开启")
                save_ota_result(False, {"error": "未找到任何蓝牙设备"})
                return
            ...
            if not user_input:
                logger.info("用户取消操作")
                print("操作已取消")
                save_ota_result(False, {"error": "用户取消操作"})
                return
            ...
            if user_input.isdigit():
                ...
                if 0 <= idx < len(all_devices):
                    ble_name, target_address = all_devices[idx]
                    logger.info(f"用户选择了设备: {ble_name} ({target_address})")
                    print(f"已选择设备: {ble_name} ({target_address})")
                else:
                    logger.error("无效的设备选择")
                    print("错误: 无效的设备编号")
                    save_ota_result(False, {"error": "无效的设备选择"})
                    return
            else:
                # 假设用户输入的是设备名称
                user_input_lower = user_input.lower()
                for name, addr in all_devices:
                    if name.lower() == user_input_lower or user_input_lower in name.lower():
                        ble_name = name
                        target_address = addr
                        logger.info(f"用户输入设备名: {user_input}, 匹配设备: {ble_name} ({target_address})")
                        print(f"已匹配设备: {ble_name} ({target_address})")
                        break
                
                if not target_address:
                    logger.error(f"未找到匹配设备: {user_input}")
                    print(f"错误: 未找到名称包含 '{user_input}' 的设备")
                    save_ota_result(False, {"error": f"未找到匹配设备: {user_input}"})
                    return
        except Exception as e:
            logger.error(f"扫描设备出错: {e}")
            print(f"错误: 扫描设备时出错: {e}")
            save_ota_result(False, {"error": f"扫描设备出错: {e}"})
            return
    else:
...
if __name__ == "__main__":
    try:
        ...
        print("  - 出错时自动减小包大小并重试")
        print("\n设备连接:")
        print("  - 优先从/home/YARBO/.ros/base.json读取设备名称")
        print("  - 如果base.json不存在或无效，将扫描并列出可用设备")
        print("  - 您可以通过输入设备序号或名称进行选择")
        print("\n示例:")
